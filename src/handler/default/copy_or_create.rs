...
pub async fn copy_or_create(request: &Request) -> teo_result::Result<Response> {
    ...
    let value: Value = request.transaction_ctx().run_transaction(|ctx: transaction::Ctx| async move {
        ...
        let object = ctx.find_unique_internal(model, request.body_value().as_ref(), true, action, Some(request.clone()), path![]).await?;
        match object {
            Some(object) => {
                let copy = binding.get("copy");
