...
pub fn fetch_expression_kind<I>(expression: &Expression, schema: &Schema, info_provider: &I, expect: &Type, namespace: &Namespace) -> Result<Object> where I: InfoProvider {
    match &expression.kind {
        ...
        ExpressionKind::EnumVariantLiteral(e) => fetch_enum_variant_literal(e, schema, info_provider, &expect.expect_for_enum_variant_literal(), namespace),
        ExpressionKind::TupleLiteral(t) => fetch_tuple_literal(t, schema, info_provider, &expect.expect_for_tuple_literal(), namespace),
        ExpressionKind::ArrayLiteral(a) => fetch_array_literal(a, schema, info_provider, &expect.expect_for_array_literal(), namespace),
        ExpressionKind::DictionaryLiteral(d) => fetch_dictionary_literal(d, schema, info_provider, &expect.expect_for_dictionary_literal(), namespace),
