...
pub fn fetch_expression<I>(expression: &Expression, schema: &Schema, info_provider: &I, expect: &Type, namespace: &Namespace) -> Result<Value> where I: InfoProvider {
...
pub fn fetch_expression_or_default<I, T>(expression: Option<&Expression>, schema: &Schema, info_provider: &I, default: T, expect: &Type, namespace: &Namespace) -> Result<Value> where I: InfoProvider, T: Into<Value> {
...
pub fn fetch_expression_or_null<I>(expression: Option<&Expression>, schema: &Schema, info_provider: &I, expect: &Type, namespace: &Namespace) -> Result<Value> where I: InfoProvider {
    if let Some(expression) = expression {
        fetch_expression(expression, schema, info_provider, expect, namespace)
    } else {
...
pub fn fetch_dictionary_key_expression<I>(expression: &Expression, schema: &Schema, info_provider: &I, namespace: &Namespace) -> Result<Value> where I: InfoProvider {
    if let Some(value) = expression.resolved().value() {
        Ok(Value::from(value.clone()))
    } else {
