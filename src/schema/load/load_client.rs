...
pub fn load_client(dest_namespace: &mut Namespace, schema: &Schema, client: &Config, diagnostics: &mut Diagnostics) -> Result<()> {
    ...
    let provider: ClientLanguage = fetch_expression_or_null(client.get_item("provider"), schema, client, provider_expect)?.try_into()?;
    let dest: String = fetch_expression_or_null(client.get_item("dest"), schema, client, dest_expect)?.try_into()?;
    let package: bool = fetch_expression_or_default(client.get_item("package"), schema, client, true, package_expect)?.try_into()?;
    let host: String = fetch_expression_or_null(client.get_item("host"), schema, client, host_expect)?.try_into()?;
    let object_name: String = fetch_expression_or_default(client.get_item("objectName"), schema, client, "teo", object_name_expect)?.try_into()?;
    let git_commit: bool = fetch_expression_or_default(client.get_item("gitCommit"), schema, client, false, git_commit_expect)?.try_into()?;
