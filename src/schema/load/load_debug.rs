...
pub fn load_debug(main_namespace: &namespace::Builder, schema: &Schema, debug: &Config, diagnostics: &mut Diagnostics) -> Result<()> {
    ...
    let dest_namespace = main_namespace.descendant_namespace_or_create_at_path(&debug.namespace_string_path());
