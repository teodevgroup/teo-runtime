...
pub fn load_debug(main_namespace: &mut Namespace, schema: &Schema, debug: &Config, diagnostics: &mut Diagnostics) -> Result<()> {
    ...
    let log_queries: bool = fetch_expression_or_null(debug.get_item("logQueries"), schema, debug, expect, main_namespace)?.try_into()?;
    let log_migrations: bool = fetch_expression_or_null(debug.get_item("logMigrations"), schema, debug, expect, main_namespace)?.try_into()?;
    let log_seed_records: bool = fetch_expression_or_null(debug.get_item("logSeedRecords"), schema, debug, expect, main_namespace)?.try_into()?;
