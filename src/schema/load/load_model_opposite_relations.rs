...
pub(super) fn load_model_opposite_relations(main_namespace: &NamespaceBuilder) {
    let relations = fetch_model_opposite_relations(main_namespace);
    main_namespace.model_opposite_relations_map = relations;
}

fn fetch_model_opposite_relations(main_namespace: &NamespaceBuilder) -> BTreeMap<Vec<String>, Vec<(Vec<String>, String)>> {
    let mut result = btreemap! {};
    add_model_opposite_relations_from_namespace(main_namespace, &mut result);
    result
}

fn add_model_opposite_relations_from_namespace(namespace: &NamespaceBuilder, result: &mut BTreeMap<Vec<String>, Vec<(Vec<String>, String)>>) {
