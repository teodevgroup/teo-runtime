...
impl Ctx {
    ...
    pub async fn find_unique<'a, T: From<model::Object>>(&'static self, model: &'static Model, finder: &'a Value, req_ctx: Option<request::Ctx>) -> Result<Option<T>> {
    ...
    pub async fn find_first<'a, T: From<model::Object>>(&'static self, model: &'static Model, finder: &'a Value, req_ctx: Option<request::Ctx>) -> Result<Option<T>> {
    ...
    pub async fn find_many<'a, T: From<model::Object>>(&'static self, model: &'static Model, finder: &'a Value, req_ctx: Option<request::Ctx>) -> Result<Vec<T>> {
    ...
    pub(crate) async fn find_unique_internal(&self, model: &Model, finder: &Value, ignore_select_and_include: bool, action: Action, req_ctx: Option<request::Ctx>) -> Result<Option<model::Object>> {
        let transaction = self.transaction_for_model_or_create(model)?;
        transaction.find_unique(model, finder, ignore_select_and_include, action, req_ctx).await
    }

    pub(crate) async fn find_first_internal<'a>(&'static self, model: &'static Model, finder: &'a Value, ignore_select_and_include: bool, action: Action, req_ctx: Option<request::Ctx>) -> Result<Option<model::Object>> {
        let transaction = self.transaction_for_model_or_create(model)?;
    ...
    pub(crate) async fn find_many_internal<'a>(&'static self, model: &'static Model, finder: &'a Value, mutation_mode: bool, action: Action, req_ctx: Option<request::Ctx>) -> Result<Vec<model::Object>> {
        let transaction = self.transaction_for_model_or_create(model)?;
        transaction.find_many(model, finder, mutation_mode, action, req_ctx).await
    }

    pub(crate) async fn batch<'a, F, Fut>(&'static self, model: &'static Model, finder: &'a Value, action: Action, req_ctx: Option<request::Ctx>, f: F) -> Result<()> where
    ...
    pub(crate) async fn count<'a>(&'static self, model: &'static Model, finder: &'a Value) -> Result<usize> {
        let transaction = self.transaction_for_model_or_create(model)?;
        transaction.count(model, finder).await
    }

    pub(crate) async fn aggregate<'a>(&'static self, model: &'static Model, finder: &'a Value) -> Result<Value> {
        let transaction = self.transaction_for_model_or_create(model)?;
        transaction.aggregate(model, finder).await
    }

    pub(crate) async fn group_by<'a>(&'static self, model: &'static Model, finder: &'a Value) -> Result<Value> {
        let transaction = self.transaction_for_model_or_create(model)?;
    ...
    pub(crate) fn new_object(&'static self, model: &'static Model, action: Action, req_ctx: Option<request::Ctx>) -> Result<model::Object> {
        Ok(model::Object::new(req_ctx, self.clone(), model, action))
    }

    pub(crate) async fn new_object_with_teon_and_path<'a>(&'static self, model: &'static Model, initial: &Value, path: &KeyPath, action: Action, req_ctx: Option<request::Ctx>) -> Result<model::Object> {
        let object = self.new_object(model, action, req_ctx)?;
        object.set_teon_with_path(initial, path).await?;
        Ok(object)
    }

    pub async fn create_object(&'static self, model: &'static Model, initial: impl Borrow<Value>, req_ctx: Option<request::Ctx>) -> Result<model::Object> {
